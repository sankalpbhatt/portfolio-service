    ...
    public static Specification<Role> getRoleByCriteria(RoleSearchCriteria searchCriteria) {
        return (root, query, builder) -> {
            ...
            switch (name.getOperation()) {
                case EQUALS ->
                        predicate = builder.and(predicate, builder.equal(builder.lower(root.get("name")), name.getValue()));
                case CONTAINS ->
                        predicate = builder.and(predicate, builder.like(builder.lower(root.get("name")), "%" + name.getValue() + "%"));
                case STARTS_WITH ->
                        predicate = builder.and(predicate, builder.like(builder.lower(root.get("name")), "%" + name.getValue()));
                case ENDS_WITH ->
                        predicate = builder.and(predicate, builder.like(builder.lower(root.get("name")), name.getValue() + "%"));
