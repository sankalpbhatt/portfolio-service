...
public class UserInfoSpecification {
    ...
    public static Specification<UserInfo> getUserInfoBySpecification(UserInfoSearchCriteria searchCriteria) {
        return (root, query, builder) ->
        {
            ...
            if (Objects.nonNull(nameFilter)) {
                switch (nameFilter.getOperation()) {
                    case EQUALS -> {
                        Predicate firstName = builder.equal(builder.lower(root.get("firstName")), nameFilter.getValue());
                        Predicate lastName = builder.equal(builder.lower(root.get("lastName")), nameFilter.getValue());
                        predicate = builder.or(firstName, lastName);
                    }
                    case CONTAINS -> {
                        Predicate firstName = builder
                                .like(builder.lower(root.get("firstName")), "%" + nameFilter.getValue() + "%");
                        Predicate lastName = builder
                                .like(builder.lower(root.get("lastName")), "%" + nameFilter.getValue() + "%");
                        predicate = builder.or(firstName, lastName);
                    }
                    case STARTS_WITH -> {
                        Predicate firstName = builder
